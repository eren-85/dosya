...
class BybitSource(ExchangeDataSource):
    """Bybit exchange data provider"""
    ...
    async def fetch_ohlcv(self, symbol: str, timeframe: str, limit: int = 500,
        ...
        async with self._rate_limiter:
            try:
                ...
                print(f"Error fetching OHLCV from Bybit: {e}")
                return []
    ...
    async def fetch_recent_trades(self, symbol: str, limit: int = 100) -> List[TradeData]:
        async with self._rate_limiter:
            try:
                ...
                print(f"Error fetching trades from Bybit: {e}")
                return []
    ...
    async def fetch_ticker(self, symbol: str) -> Dict[str, Any]:
        async with self._rate_limiter:
            try:
                ...
                print(f"Error fetching ticker from Bybit: {e}")
                raise
    ...
    async def fetch_funding_rate(self, symbol: str) -> Optional[Dict[str, Any]]:
        async with self._rate_limiter:
            try:
                ...
                print(f"Error fetching funding rate from Bybit: {e}")
                return None
    ...
    async def fetch_open_interest(self, symbol: str) -> Optional[Dict[str, Any]]:
        async with self._rate_limiter:
            try:
                ...
                print(f"Error fetching open interest from Bybit: {e}")
                return None
