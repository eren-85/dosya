...
class AdvancedDataCollector:
    ...
    # OB max age thresholds (dashboard/backfill tolerant values)
    # Based on timeframe - larger TF = wider tolerance
    DEFAULT_OB_MAX = {
        "1m":  60_000,      # 60 seconds
        "3m":  120_000,     # 2 minutes
        "5m":  240_000,     # 4 minutes
        "15m": 600_000,     # 10 minutes
        "30m": 1_200_000,   # 20 minutes
        "1h":  3_600_000,   # 60 minutes (1 hour)
        "4h":  7_200_000,   # 120 minutes (2 hours)
        "1d":  21_600_000,  # 6 hours
        "1w":  86_400_000,  # 24 hours (OB rarely useful at this TF)
        "1M":  259_200_000  # 3 days (OB rarely useful at this TF)
    }

    def __init__(self, symbol: str, timeframe: str, market: str = 'futures', exchanges: List[str] = None, ob_max_age_ms: int = None):
        self.symbol = symbol
        ...
        # Log OB max age
        ob_age_display = self.ob_max_age_ms if self.ob_max_age_ms else self.DEFAULT_OB_MAX.get(timeframe, 300_000)
        logger.info(f"   OB max age: {ob_age_display / 1000:.1f}s ({ob_age_display / 60_000:.1f}m)")
    ...
    def attach_ob_to_nearest_bar(self, df: pd.DataFrame, ob_snapshot: Dict, max_age: int = None) -> pd.DataFrame:
        """
        ...
        Args:
            ...
            ob_snapshot: Order Book snapshot dict
            max_age: Max age in milliseconds (defaults to TF-based threshold)
        """
        if not ob_snapshot or df.empty:
            return df

        # Use provided max_age or fallback to DEFAULT_OB_MAX
        if max_age is None:
            max_age = self.DEFAULT_OB_MAX.get(self.timeframe, 300_000)  # Default: 5 minutes
    ...
    def collect_all(
        ...
        if ob:
            df = self.attach_ob_to_nearest_bar(df, ob, max_age=self.ob_max_age_ms)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ob-max-age-ms', type=int, default=None,
                        help='Order Book max age in milliseconds (auto-detect from TF if not set)')
    ...
    def process_symbol(symbol: str, idx: int, total: int):
        ...
        try:
            collector = AdvancedDataCollector(
                ...
                ob_max_age_ms=args.ob_max_age_ms
            )
